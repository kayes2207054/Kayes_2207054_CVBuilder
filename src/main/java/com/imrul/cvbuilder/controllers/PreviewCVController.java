...
public class PreviewCVController {
    ...
    private void handleSaveCV() {
        if (currentCV == null) {
            showAlert("Error", "No CV to save", Alert.AlertType.ERROR);
            return;
        }

        // Use DatabaseManager with concurrency to save CV asynchronously
        DatabaseManager.getInstance().saveCVAsync(currentCV, (exception) -> {
            Platform.runLater(() -> {
                showAlert("Error", "Failed to save CV: " + exception.getMessage(), Alert.AlertType.ERROR);
            });
        }).thenAccept(success -> {
            Platform.runLater(() -> {
                if (success) {
                    if (currentCV.getId() != null && currentCV.getId() > 0) {
                        showAlert("Success", "CV updated successfully!", Alert.AlertType.INFORMATION);
                    } else {
                        showAlert("Success", "CV saved successfully!", Alert.AlertType.INFORMATION);
                    }
                } else {
                    showAlert("Error", "Failed to save CV to database", Alert.AlertType.ERROR);
                }
            });
        }).exceptionally(throwable -> {
            Platform.runLater(() -> {
                showAlert("Error", "An error occurred while saving: " + throwable.getMessage(), Alert.AlertType.ERROR);
            });
            return null;
        });
    }
